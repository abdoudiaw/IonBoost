...
contains
    ...
    !   Determine the length of the plasma
      if(.not.lfini) then
         ...
         cs=sqrt(cs2)
         if(profil.eq.'sack')length=10._dp*LSS+25._dp*cs
         if(profil.eq.'expo')length=LSS+25._dp*cs
         if(profil.eq.'step')length=25._dp*cs
        ! if(itmax.eq.1) goto 91
         Th=Thmax*Thot_(multiphase,trise,0._dp)
         Tc=Tcmax*Tcold_(0._dp)
        !
        do 90 itime=1,itmax
    ...
    !   mesh
        ...
        do i = 0,1
           niSS(i)=ni0
           if(profil.eq.'sack')  niSS(i)=ni0*(2._dp/pi)*DATAN(exp(-x0(i)/LSS))
        ...
        do i = 2,ncell
           ...
           if((profil.eq.'expo'.or.profil.eq.'sack').and.dx0(i).gt.dx0(i-1).and.dx0(i).gt.LSS/5._dp) dx0(i)=LSS/5._dp
           if(profil.eq.'gaus' .and.dx0(i).gt.dx0(i-1).and.dx0(i).gt.lmax/5._dp) dx0(i)=lmax/5._dp
           !
           x0(i)=x0(i-1)+dx0(i)
           niSS(i)=ni0
